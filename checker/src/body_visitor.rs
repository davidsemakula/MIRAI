...
pub struct BodyVisitor<'analysis, 'compilation, 'tcx> {
    ...
    pub already_reported_errors_for_multi_span: HashSet<(MultiSpan, DiagnosticMessage)>,
...
impl<'analysis, 'compilation, 'tcx> BodyVisitor<'analysis, 'compilation, 'tcx> {
    ...
    pub fn new(
        ...
        BodyVisitor {
            ...
            already_reported_errors_for_multi_span: HashSet::new(),
    ...
    pub fn emit_diagnostic(&mut self, diagnostic_builder: DiagnosticBuilder<'compilation, ()>) {
        ...
        // Deduplicate diagnostics by mulispan and primary message combination.
        let multi_span = MultiSpan::from_spans(
            diagnostic_builder
                .span
                .primary_spans()
                .iter()
                .chain(
                    diagnostic_builder
                        .children
                        .iter()
                        .flat_map(|sub_diag| sub_diag.span.primary_spans()),
                )
                .cloned()
                .collect(),
        );
        let primary_msg = diagnostic_builder.messages[0].0.clone();
        let diagnostic_span_message = (multi_span, primary_msg);
        if self
            .already_reported_errors_for_multi_span
            .contains(&diagnostic_span_message)
        {
            diagnostic_builder.cancel();
            return;
        }
        ...
        self.already_reported_errors_for_multi_span
            .insert(diagnostic_span_message);
    }
